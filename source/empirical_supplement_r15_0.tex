...
This supplement documents the public-source lineage, raw July audit, comparison design, support diagnostics, resampling, initiation-stream evidence, current-data denominator audit, institutional-source check, and reproduction contract. It is organized around the empirical claims in the main paper. Extended identified-set proofs, constrained-program certificates, and formal-verification material are kept in the separate \emph{Legacy Mathematical and Formal-Verification Archive} and are not appended to the professor-facing paper.
\end{plainbox}
...
\section{Claim and evidence hierarchy}
...
\section{Annual public-source lineage}

\subsection{Dataset binding}
...
\subsection{Official metadata and changelog}
...
\section{State matrix and estimator}
...
\end{equation}
The simple amplitude is $M_{\max}=\max_{t,j}|G_{tj}|$. The event window begins 29 August 2021 and contains ten twelve-hour bins; the baseline is seven days earlier.
...
The full-count statistic drops the stratum intersection and weights. For each half-day it compares unconditional state shares and again takes the maximum absolute cell contrast. Two reference sets are reported:
\begin{itemize}
...
\end{itemize}
The second is the support-independent comparison for the post-change measurement product.

\section{Common-support audit}
...
The same multinomial logic is applied to unconditional half-day state counts. Ida's full-count interval is $[0.4754,0.6015]$. The strongest post-change reference point is 0.3098 with interval $[0.2599,0.3799]$; the remaining top-five intervals have upper endpoints between 0.3636 and 0.3843. For each of 4,000 draws, Ida is ranked against all 151 independently resampled post-change references. Rank 1 occurs in 100 percent of draws, and the reported rank interval is $[1,1]$.
...
\section{Thresholds, secondary statistics, and excluded episodes}
...
Excluded episodes are not members of the ordinary-week rank. Pre-break events cannot reproduce a positive $J_{01}$ contrast because that state is absent from their public regime. The large post-Ida statistic compares a recovery week with Ida itself. Francine is much smaller in the later regime. These comparisons reinforce the need to distinguish event stress from the measurement object used to observe it.

\section{Officer-initiation evidence}
...
\section{Current-data denominator audit}
...
Full-count Ida maximum & 0.5320; rank 1/151 & \path{r15_raw_window_scores.csv} \\
Support-independent post-change rank & 1/152 including Ida & \path{r15_aggregate_diagnostics.json} \\
