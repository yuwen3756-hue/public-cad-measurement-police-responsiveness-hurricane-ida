...
\fancyhead[L]{\footnotesize\itshape Mathematical appendix: workload, observation, and identification}

\title{\vspace{-1.6em}\textbf{Mathematical Definitions, Identification Results, and Reproduction Guide}\\[0.35em]
\large Appendix to \emph{From Police Workload to Auditable Performance Measures}}
\author{}
\date{24 August 2026\\[0.2em]\small Appendix R12.2 \quad Results R12}
...
This appendix connects a police-workload layer to two mathematical modules. Section~\ref{sec:measurement_map} defines the reported-demand denominator, the public observation map, measure-specific identified sets, and target-specific witness minimality. Sections~\ref{sec:observables}--\ref{sec:reference} define the public-record stress-test statistic, and Section~\ref{sec:disposition} localizes the Ida pattern. Section~\ref{sec:structural} applies the identified-set logic to the internal/public bridge, queue, priority, DV classification, endpoint-selected response time, and follow-through. Section~\ref{sec:frontier} retains the prospective multi-model frontier. Section~\ref{sec:m8p} formalizes the 2025--2026 measurement-regime replay. Sections~\ref{sec:repro}--\ref{sec:claims} record numerical invariants, reproduction commands, and interpretation rules. Every reported empirical value comes from the locked aggregate artifacts.
\end{plainbox}
...
The released call count is likewise a measure of $D_t^{\mathrm{CAD}}$, rather than a direct measure of $D_t^*$ or either intermediate stage in \cref{eq:demand_chain}. R12.2 estimates no DV call-volume trend. Its DV application begins with a declared CAD risk set and derives the additional witnesses needed to measure service conditional on that set.
...
These are target-specific results. Calls-holding transitions enter the minimum bundle when the target is queue delay. Realized unit availability enters when the target includes capacity or capacity-adjusted responsiveness. A fully observed receipt-to-qualified-endpoint distribution uses neither. Wherever $w_L$ appears, the privacy-safe product must retain the indicated joint cells; marginal aggregates alone leave the pairing unresolved, although no event identifier need be released.

\subsection{The stress-test statistic is an observable diagnostic}

The Ida statistic belongs to the released-record side of \cref{eq:observation_map}. Let $G(Y)$ be the standardized ten-bin public-topology matrix defined below. Then
\begin{equation}
T(Y):=U_{\mathrm{full}}(G(Y))
\label{eq:stress_diagnostic}
\end{equation}
is a constrained discrepancy statistic applied identically to Ida and every qualified reference window. It measures whether the released field relationships remain close to the frozen support class. It does not replace $m_k(S)$.
...
\subsection{Composition-only exclusion theorem}

\begin{plainbox}[title=Theorem 1: composition-only sign restriction]
...
\end{equation}
Thus opposite-signed $\Delta H_{11,r,t}$ and $\Delta H_{01,r,t}$ exclude composition-only in that cell.
\end{plainbox}

This result requires no stochastic test and no choice among decomposition orders. It is a finite-file accounting implication.

\subsection{Six independent sign witnesses}
...
\item rank 1 in all three reference designs;
...
\item The public-state configuration changes after midday on 31 August: $\Jzeroone$ becomes positive while $\Joneone$ remains negative.
\item Recorded-disposition composition alone is excluded in the six focal GOA/RTF/NAT cells.
