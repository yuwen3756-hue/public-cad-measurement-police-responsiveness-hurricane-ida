...
\title{\vspace{-1.6em}\textbf{Online Mathematical and Reproduction Supplement}\\[0.35em]
\large Appendix to \emph{Public CAD Is Not Operational Ground Truth}}
\author{}
\date{24 August 2026\\[0.2em]\small Appendix R14.0 \quad Results R14}
...
This online supplement preserves the mathematical and reproducibility material behind R14.0. The main paper now uses the simple maximum absolute cell contrast as its headline statistic and treats the constrained LP as robustness. Section~\ref{sec:measurement_map} defines the reported-demand denominator, observation map, and measure-specific identified sets. Sections~\ref{sec:observables}--\ref{sec:reference} document the standardized matrix, reference calibration, and legacy restricted-support score. Section~\ref{sec:disposition} gives the within-disposition accounting. Later sections retain the public/internal bridge, queue, priority, classification, endpoint-selection, 2025--2026 replay, finite verification, and exact reproduction boundaries. R14 aggregate diagnostics and resampling outputs are built by \path{scripts/build_r14_evidence.py}. Every empirical value is derived from packaged aggregate artifacts.
\end{plainbox}
